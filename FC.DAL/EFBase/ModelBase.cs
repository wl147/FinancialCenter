...
namespace FC.DAL.EFBase
{
        ...
        public void ModelAdapt(object source)
        {
            ...
            targetProperties.ForEach(new Action<PropertyInfo>(delegate (PropertyInfo tPI)
            {
                if (tPI.CanWrite)
                {
                    ...
                    {
                        #region 模型集合映射
                        var attributes = tPI.GetCustomAttributes(typeof(ModelsMap), true);
                        if (attributes != null && attributes.Count() > 0)
                        {
                            var attr = attributes.Select(att => att as ModelsMap).FirstOrDefault(att => att.SourceType == source.GetType());
                            if (attr != null)
                            {
                                ...
                                if (sourceDatas != null && sourceDatas.GetType().GetInterface("IEnumerable") != null)
                                {
                                    ...
                                    while (enumerator.MoveNext())
                                    {
                                        try
                                        {
                                            var m = (ModelBase)Activator.CreateInstance(attr.ModelType);
        ...
        public T ModelToEntity<T>()
        {
            ...
            sourceProperties.ForEach(new Action<PropertyInfo>(delegate (PropertyInfo sourcePI)
            {
                if (sourcePI.CanRead)
                {
                    ...
                    {
                        #region 模型集合映射
                        var attributes = sourcePI.GetCustomAttributes(typeof(ModelsMap), true);
                        if (attributes != null && attributes.Count() > 0)
                        {
                            var attr = attributes.Select(att => att as ModelsMap).FirstOrDefault(att => att.SourceType == entity.GetType());
                            if (attr != null)
                            {
    ...
    /// </summary>
    [AttributeUsage(AttributeTargets.Property, AllowMultiple = true)]
    public class ModelsMap : Attribute
    {
        /// <summary>
        /// 元数据类型
        /// </summary>
